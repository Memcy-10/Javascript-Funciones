...
function calcularValorParte(parte) {
        let resultado = parte.cantidad * parte.precio_unitario
...
function calcularTotalInventario(listado) {
    ...
    listado.forEach(Element=>{
        total = total + calcularValorParte(Element)
    }
)
    // console.log(total)
    return total
...
// console.log(calcularTotalInventario(partes))
...
function registrarEntrada(idParte, cantidad) {
    ...
    if ((cantidad + parte.cantidad) <= parte.capacidad_maxima) {
        parte.cantidad += cantidad
        // console.log(parte.cantidad)
        return true;
    }
    else{
        return false
...
// registrarEntrada("AND-MECH-001",20)
...
// registrarSalida("AND-MECH-001",50)
// registrarSalida("AND-MECH-001",30)
...
function calcularPorcentajeOcupacion(parte) {
    ...
    //     let porcentaje = (parte.cantidad*100) / parte.capacidad_maxima
    //     return porcentaje
    // })
        let porcentaje = (parte.cantidad*100) / parte.capacidad_maxima
...
// calcularPorcentajeOcupacion(partes)
...
function actualizarReloj() {
    ...
    return document.getElementById("reloj").textContent = hh + ":" + mm + ":" + ss + "Z";
}
